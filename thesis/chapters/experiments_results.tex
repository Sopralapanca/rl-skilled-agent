...
To solve this issue different strategies have been tried like:
\begin{itemize}
    \item \textbf{Dropout}: adding a dropout layer after the linear adapters for the skills, after the linear adapter for the state, or after the softmax layer for the weights.
    Dropout randomly set some weights to zero during training to avoid the collapse.
    \item \textbf{Batch Normalization}: adding a batch normalization layer after the linear adapters for the skills, or after the linear adapter for the state.
    \item \textbf{Sigmoid}: using a sigmoid activation function instead of ReLU in the adapters.
    \item \textbf{Entropy Penalty}: add a regularization term to the loss function of PPO based on the entropy of the weights.
    This encourages the weights to be more uniform.
\end{itemize}

Unfortunately, none of these strategies seems to work.
Most of the times the weights still collapse on a single model, while adding too much regularization lead to a uniform distribution of the weights at each step.
In other cases, the agent was not able to learn.
This problem may suggest that the FMs are already too informative for these two games.
They learn the environment well, and the agent does not need to use all of them to make decisions.
